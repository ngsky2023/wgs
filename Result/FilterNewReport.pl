...
my $historyDir = "/share/work1/wangrr/DB/history398/";
GetOptions(
	"input=s{1,}"=>\@input,
	"inputdir:s"=>\$inputdir,
	"outdir:s"=>\$outdir,
...
my $history = "$historyDir/count.txt";
...
######################################################################
my %hotspots;
open IN , "/share/work3/wangrr/DB/hg19/cancer_hotspots.tsv";
<IN>;
while (<IN>){
	chomp;
...
close IN;
open IN , "/share/work3/wangrr/DB/363/must.txt";
<IN>;
while (<IN>){
	chomp;
	my @F = split /\t/ , $_;
...
while($j<@pairs){
	open(MUT,"$inputdir/somatic_$pairs[$j]/1.Annot/somatic_$pairs[$j].annovar.new.xls")||die "$!";
	if ($mode eq "putative_somatic"){
		 if (! -d "$historyDir/somatic_$pairs[$j]/1.Annot/" and -e "$inputdir/somatic_$pairs[$j]/1.Annot/somatic_$pairs[$j].annovar.new.xls"){
			`mkdir -p $historyDir/somatic_$pairs[$j]/1.Annot/`;
			`cp $inputdir/somatic_$pairs[$j]/1.Annot/somatic_$pairs[$j].annovar.new.xls $historyDir/somatic_$pairs[$j]/1.Annot/`;
		}
	}
	while(defined($a=<MUT>)){
		chomp $a;
...
while(defined($a=<ANNRAW>)){
	...
	if($a=~/SampleName/){
		...
		next if  $b[11] eq 'ncRNA_exonic' or $b[11] eq 'intronic';
		...
		if($mode eq "somatic"){
			...
			if (exists $history{$key}){
				$freq=$history{$key}->[2];
				next if (($freq>=0.05 or $b[9]>=0.4) and $tsgr{$b[12]}>500 and $ishotspots == 0);
			}else{
				$freq=$count{$locus}/($#countsamp+1);
				next if ($count{$locus}>=2 and ($freq>=0.1 or $b[9]>=0.4) and $tsgr{$b[12]}>500 and $ishotspots == 0);
			}
