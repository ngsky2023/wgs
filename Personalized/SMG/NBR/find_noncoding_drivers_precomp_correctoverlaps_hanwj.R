...
PATH_TO_DATA_AND_GENOME = "/share/Oncology/RD_HCC/IBFC2018028/WgsReSeq/result/snp/noncodingdriver/NBR/NBR/NBR_TO_SHARE" # Set this path accordingly.
genomeFile              = paste(PATH_TO_DATA_AND_GENOME,"/genome.fa",sep="") # Set genome file accordingly
#genomeFile              = "/Users/fa8/Desktop/PANCANCER/TRANSCRIPTION_VS_INDELS_ANALYSIS/hs37d5.fa" # Set genome file accordingly

max_num_muts_perRegion_perSample = 2
unique_indelsites_FLAG = 1
load(paste(PATH_TO_DATA_AND_GENOME,"/GRanges_driver_regions.RData",sep="")) # Regions to be excluded for the background model fit (GRanges object called "gr_drivers")
trinucfreq_neutralbins_file = paste(PATH_TO_DATA_AND_GENOME,"/Trinucfreqs_within_100kb_bins.txt",sep="")
regions_neutralbins_file = paste(PATH_TO_DATA_AND_GENOME,"/Neutral_regions_within_100kb_bins.txt",sep="")
...
write.table(targetregions_df, file=paste(trinucfreqs_prefix,"-Selection_output.txt",sep=""), sep = "\t", row.names = FALSE, col.names = TRUE, append=FALSE, quote=FALSE)


# Plot of the underlying gamma distributions of the 4 models (subs and indels with and without covariates)

pdf(paste(trinucfreqs_prefix,"-Underlying_gamma_distributions.pdf",sep=""),height=4,width=4)
