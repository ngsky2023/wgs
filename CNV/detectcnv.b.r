...
detectCNV_core <- function(alldata,parameter,ideoCyto){
  ...
  mhtplot <- function(out, da, name, hasxy) {
    ...
    Encoding(Ylab) = 'UTF-8'
    lbl = c(1.5:2.5)
    plot(x.all, y.all, type='n', main=Name, cex.main=2, cex.lab=2, ylim=c(1.5, 2.5), axe=F, 
  ...
  scatterplot <- function(outdir, da, name, sex, ideoCyto) {
    ...
    for (i in 1:chrNum) {
      ...
      Xlab = 'Position (Mb)'
      Encoding(Xlab) = 'UTF-8'
      plot(x.cur, y.cur, ylim=c(-1,3), type='n', main=Name, cex.main=2, cex.lab=2, xlab=Xlab, 
