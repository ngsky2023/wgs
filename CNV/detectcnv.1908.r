...
parameter$Ncores = 20
parameter$Totalbin = 12500
parameter$Nbin = parameter$Totalbin / parameter$nStep
parameter$window_len = 20000
...
parameter$SW_cutoff = 3             # SW cutoff
...
parameter$merge_cutoff = 0.02        # merge cutoff, if copy number difference in two adjacent cnv, then merge it.
parameter$ra_off = 0.6
parameter$cn_off = 0.03
...
num_file = length(parameter$file_20K);


if(num_file != length(parameter$file_GC) || num_file < 1) {
  stop("[error] no files or the 20K.GC file number donot equal 20K file number\n")
}
...
detectCNV_core <- function(alldata,parameter,ideoCyto){
  ...
  combine_segments <- function(segments,fd,rdata,ref_data,score,xcoor,distance_ref,yh){
    ...
    for(i in 1:length(change)){
      if(change[i] > merge_cutoff){
        ...
        if(((abs(copy[i]-yh)>cn_off & abs(copy[i+1]-yh)>cn_off)|(abs(copy[i]-yh)<cn_off & abs(copy[i+1]-yh)<cn_off)) & change[i] < change_max){
          com_point = i
          change_max = change[i]
        }
      }
  ...
  mhtplot <- function(out, da, name, hasxy) {
    ...
    Encoding(Ylab) = 'UTF-8'
    lbl = c(1:3)
    plot(x.all, y.all, type='n', main=Name, cex.main=2, cex.lab=2, ylim=c(1, 3), axe=F, 
  ...
  Call_cnv_GD <- function(xcoor, ldata,rdata,ref_data, i, sex){
    ...
    rdata = rdata * nor_ratio
    ref_data = ref_data * nor_ratio
  ...
  calculate_fd <- function(data,chr_ratio,reference,sex){
    
    info = homo_supplement(reference,data)
    data = info$data
    ...
    fd.data[fd.data > 6] = NA
    
    return(list(data = data, fd.data = fd.data, supplement = supplement,reference = reference))
  }
  ...
  chr_ratio = alldata$chr_ratio
  nor_ratio = alldata$nor_ratio
  ...
  data_info = calculate_fd(data,chr_ratio,reference,sex)
  
  data = data_info$data
  fd.data = data_info$fd.data
  supplement = data_info$supplement
...
do_preProcess <- function(filepath_20K,parameter){
  ...
  smooth_spline_Fit <- function(outprefix, rc, gc, deter){
    ...
    smooth.fittedRC  <- predict(smooth_model, gc[order(gc)])$y

    if(!parameter$IT_version){

      png(file=paste(outprefix, '.GCcorrelation_before.png', sep=''), type='cairo', height=600, width=900)
      par(mar=c(5.1,4.5,4.1,2.1), mgp=c(3,1,0), ps=15)
      plot(gc2, rc2, main='Correlation between GC% and Read count', xlab='CG%', ylab='Read count', col='#A0522D',
         cex.lab=1.5, cex.main=1.5, cex.axis=1, cex.sub=1, cex=0.5, font.lab=1.2, font.main=2, font.axis=1, lwd=1,
         ylim=c(0, max(200, na.rm=T)), xlim=c(10, 90), pch=21)
      lines(gc[order(gc)], smooth.fittedRC, col=3, lwd=2)
      M <- median(rc)
      segments(min(gc), M, max(gc), M, col=1, lwd=2, lty=2)
      legend('topright', c('smooth.spline'), col=c(3,4), lty = 1, pch = 20, inset = .02)
      dev.off()
  ...
  remove_low_coverage <- function(da_rd,da_gc,reference,nBlock,nStep){
    ...
    reference[reference < 0.01] = NA
    #cutoff = limit_cutoff * apply(reference,1,mean,na.rm=T)
    #for(i in 1:24){
    #  temp = reference[i,] < cutoff[i]
    #  reference[i,temp] = NA
    #}
    
    da_gc <- round(da_gc/(da_rd*36)*100, 3)
    da_gc[is.na(reference) | (da_gc > 70 | da_gc < 25)] <- NA 
    da_rd[is.na(da_gc)] <- NA
    
    return(list(da_rd = da_rd, da_gc = da_gc))
  }
  
  smooth_spline_gc_correction <- function(da_rd,da_gc,deter,sampleName,output_dir){
    
    RCgcCorrection.tatol.reads <- sum(da_rd,ma.rm=T)
    normal_count <- sum(as.numeric(da_rd))  
    
    outprefix <- paste(output_dir, "/", sampleName, sep="")
    MedianRC <- median(da_rd[1:22,], na.rm=T)
    smooth_model <- smooth_spline_Fit(outprefix, da_rd, da_gc,deter)
    ...
    if(!parameter$IT_version){
      png(file=paste(outprefix, '.GCcorrelation_after.png', sep=''), type='cairo', height=600, width=900)
      plot(da_gc, as.vector(RCgcCorrection), main='after Correlation between GC% and Read count',
          xlab='CG%', ylab='Read count', col='#A0522D', cex.lab=1.5, cex.main=1.5, cex.axis=1, cex.sub=1, cex=0.5,
          font.lab=1.2, font.main=2, font.axis=1, lwd=1, ylim=c(0, max(200, na.rm=T)), xlim=c(10, 90), pch=21
      )
      dev.off()
    }
    ...
    if(parameter$IT_version){
      Matrixoutfile <- paste(outprefix, '.roll', sep='')
    }else{
      Matrixoutfile <- paste(outprefix, '.roll.txt', sep='')
    }

    if (file.exists(Matrixoutfile)) unlink(Matrixoutfile)
    cat(paste('#uniqMapped', round(RCgcCorrection.tatol.reads, 2), sep='\t'), end='\n', file=Matrixoutfile)
    row.names(da_rd) = chrs
    write.table(da_rd, file=Matrixoutfile, col.names=F, row.names=T, append=T, quote=F, sep='\t')
    
    return(RCgcCorrection)
  }
  
  DeAR_correction <- function(da_rd){
    
    tongxiang_deter <- function(copy_number){
      daxiao = sapply(1:22,function(x) ifelse(copy_number[x]>2,1,-1))
      fangxiang = sapply(1:22,function(x) ifelse(parameter$DeAR_direction[x]>0,1,-1))
      return(daxiao * fangxiang)
    }
    
    adjust_direction <- function(copy_number){
      bound = sapply(1:22,function(x) ifelse(copy_number[x]<2,parameter$DeAR_haploid[x],parameter$DeAR_triploid[x]))
      bili = sapply(1:22,function(x) (copy_number[x]-2)/(bound[x]-2)/abs(bound[x]-2))
      direction = parameter$DeAR_direction * (1 + bili)
      return(direction / sum(direction^2))
    }
    
    move_distance <- function(copy_number,direction,daxiao,value){
      
      tongxiang = tongxiang_deter(copy_number)
      if(sum(tongxiang) == 0){
        while(sum(tongxiang) == 0){
          copy_number = copy_number + 0.001 * direction
          tongxiang = tongxiang_deter(copy_number)
        }
        return(copy_number)
      }
    ...
    move_observe <- function(copy_number,direction){
      tongxiang = tongxiang_deter(copy_number)
      if(sum(tongxiang) == 0){
        copy_number_adjust1 = move_distance(copy_number,direction,TRUE,0)
        copy_number_adjust2 = move_distance(copy_number,-direction,TRUE,0)
      }else if(sum(tongxiang) >= 0){
        copy_number_adjust1 = move_distance(copy_number,-direction,TRUE,1)
        copy_number_adjust2 = move_distance(copy_number,-direction,TRUE,-1)
      }else{
        copy_number_adjust1 = move_distance(copy_number,direction,FALSE,1)
        copy_number_adjust2 = move_distance(copy_number,direction,FALSE,-1)
      }
      copy_number_adjust = (copy_number_adjust1+copy_number_adjust2)/2
      #bound = sapply(1:22,function(x) ifelse(copy_number_adjust[x]<2,parameter$DeAR_haploid[x],parameter$DeAR_triploid[x]))
      #copy_number_adjust = sapply(1:22,function(x) (copy_number_adjust[x]-2)/(bound[x]-2)/(bound[x]-2)+2)
      return(copy_number_adjust)
    }
    
    mean_distance <- function(copy_number,direction,value,lin){
      
      tongxiang = tongxiang_deter(copy_number)
      copy_number_adjust = copy_number
      distance = sum(sort(abs(copy_number-2))[1:15])
      deter = sum(tongxiang)
      while(deter < value){
        copy_number = copy_number + 0.001 * direction
        tongxiang = tongxiang_deter(copy_number)
        temp = sum(sort(abs(copy_number-2))[1:15])
        if(temp < distance){
          distance = temp
          copy_number_adjust = copy_number
        }
        if(lin){deter = abs(sum(tongxiang))}
        else{deter = sum(tongxiang)}
      }
      return(copy_number_adjust)
      
    }
    
    mean_observe <- function(copy_number,direction){
      tongxiang = tongxiang_deter(copy_number)
      if(sum(tongxiang) == 0){
        copy_number_adjust1 = mean_distance(copy_number,direction,5,TRUE)
        copy_number_adjust2 = mean_distance(copy_number,-direction,5,TRUE)
        distance1 = sum(sort(abs(copy_number_adjust1-2))[1:15])
        distance2 = sum(sort(abs(copy_number_adjust2-2))[1:15])
        if(distance1 > distance2){copy_number_adjust = copy_number_adjust1}
        else{copy_number_adjust = copy_number_adjust2}
      }else if(sum(tongxiang) >= 0){
        copy_number_adjust = mean_distance(copy_number,-direction,-5,FALSE)
      }else{
        copy_number_adjust = mean_distance(copy_number,direction,5,FALSE)
      }
      #bound = sapply(1:22,function(x) ifelse(copy_number_adjust[x]<2,parameter$DeAR_haploid[x],parameter$DeAR_triploid[x]))
    ...
    data = da_rd
    total_reads = sum(data[1:22,],na.rm=T)
    
    copy_number = apply(da_rd[1:22,],1,sum,na.rm=T)/sum(da_rd[1:22,],na.rm=T)*100/chromosome_ratio[1:22] * 2
    
    direction = adjust_direction(copy_number)
    copy_number_adjust = move_observe(copy_number,direction)
    #copy_number_adjust = mean_observe(copy_number,direction)
    
    bili = copy_number_adjust / copy_number
    data[1:22,] = t(sapply(1:22,function(x) data[x,]*bili[x]))
    total_reads_adjust = sum(data[1:22,],na.rm=T)
    data[23:24,] = data[23:24,] * (total_reads_adjust / total_reads)
    
    return(data)
  }
  
  adjust_aneuploid <- function(chr_ratio,chromosome_ratio){
    
    best_x = 0
    best_value = 100
    for(x in seq(-0.05,0.05,0.00001)){
      weight_piancha = (chr_ratio*(1+x)-2) * chromosome_ratio
      value = sum(weight_piancha[sort(abs(weight_piancha),index=T)$ix[1:16]])
  ...
  sumCount = sum(data)
  if(sumCount < 1000000){
    return(list(sumCount = 'None',sampleName = sampleName,sex_info = 'None'))
  }
  
  # sex type and remove low coverage
  ratio = round(sum(data[24,],na.rm=T)/sum(data[1:22,],na.rm=T)*100,3)
  sex = ifelse(ratio>0.08,'M','F')
  
  forTrain = remove_low_coverage(data,dataGC,reference[1:24,],nBlock,nStep)
  data = forTrain$da_rd
  dataGC = forTrain$da_gc
  
  sumCount_autosome = sum(data[1:22,],na.rm=T)
  ...
  # find reference and normalization
  if(sex == 'F'){
    reference = rbind(reference[1:22,],reference[25:26,])
  }else{
    reference = reference[1:24,]
  }
  chromosome_ratio = apply(reference,1,sum,na.rm=T) / sum(reference[1:22,],na.rm=T) * 100
  reference = reference * sum(data[1:22,],na.rm=T) / sum(reference[1:22,],na.rm=T)
  
  chr_ratio = apply(data,1,sum,na.rm=T) / sum(data[1:22,],na.rm=T) * 100
  ob_chr_dosage = chr_ratio / chromosome_ratio
  
  bili = 5000000 / nStep / sum(data[1:22,],na.rm=T)
  data = data * bili
  
  # GC correction and adjust chromosome dosage

  index = 1:22
  deter = index[abs(chr_ratio[1:22] - chromosome_ratio[1:22]) / chromosome_ratio[1:22] < 0.4]
  alldata = smooth_spline_gc_correction(data,dataGC,deter,sampleName,outpath)
  ob_chr_dosage = rbind(ob_chr_dosage,apply(alldata,1,sum,na.rm=T)/sum(alldata[1:22,],na.rm=T)*100/chromosome_ratio)
  GC_distance = sqrt(sum(ob_chr_dosage[2,1:22] - ob_chr_dosage[1,1:22])^2)
  
  adjust_copy = adjust_aneuploid(ob_chr_dosage[2,1:22]*2,chromosome_ratio[1:22])
  ob_chr_dosage = rbind(ob_chr_dosage,c(ob_chr_dosage[2,1:22]*(1+adjust_copy),ob_chr_dosage[2,23:24]))
  aneuploid_distance = sqrt(sum(ob_chr_dosage[2,1:22] - ob_chr_dosage[3,1:22])^2)
  
  
  if(parameter$DeAR){
    alldata = DeAR_correction(alldata)
    ob_chr_dosage = rbind(ob_chr_dosage,apply(alldata,1,sum,na.rm=T)/sum(alldata[1:22,],na.rm=T)*100/chromosome_ratio)
    DeAR_distance = sqrt(sum(ob_chr_dosage[3,1:22] - ob_chr_dosage[4,1:22])^2)
    name = paste(strsplit(sampleName,'_')[[1]][1],c('Original','GC_correction','adjust_aneuploid','DeAR_correction'),sep='_')
    rownames(ob_chr_dosage) = name
  }else{
    name = paste(strsplit(sampleName,'_')[[1]][1],c('Original','GC_correction','adjust_aneuploid'),sep='_')
    rownames(ob_chr_dosage) = name
    DeAR_distance = NA
  }
  colnames(ob_chr_dosage) = chrs
  
  if(sex == 'M'){ob_chr_dosage[,1:22] = ob_chr_dosage[,1:22]*2
  }else{ob_chr_dosage = ob_chr_dosage*2}
  ob_chr_dosage = round(ob_chr_dosage,3)
  
  chr_ratio = apply(alldata,1,sum,na.rm=T) / sum(alldata[1:22,],na.rm=T) * 100 * (1+adjust_copy)
  
  alldata = alldata * 5000000 * (1+adjust_copy) / sum(alldata[1:22,],na.rm=T)
  nor_ratio = sumCount_autosome / sum(alldata[1:22,],na.rm=T) 
  
  return(list(data = alldata, sex = sex,sampleName = sampleName, chr_ratio = chr_ratio, 
              ob_chr_dosage = ob_chr_dosage,sex_info = sex_info,DeAR_distance = DeAR_distance,
              GC_distance = GC_distance,aneuploid_distance = aneuploid_distance,sumCount = sumCount,
              nor_ratio = nor_ratio))
}
...
output_QC <- function(info){
  
  SampleName = sapply(info,function(x) x$sampleName)
  sumCount = sapply(info,function(x) x$sumCount)
  biaozhuncha = sapply(info,function(x) x$sd)
...
alldata = clusterApplyLB(cluster,filepath_20K,do_preProcess,parameter)
stopCluster(cluster)

a = sapply(alldata,function(x) if(x$sex_info!='None') cat(x$sex_info,append = T,end = '\n',file = parameter$output_sex))

message('[Call CNV ...]')

cluster <- makeCluster(parameter$Ncores,"PSOCK")
info = clusterApplyLB(cluster,alldata,detectCNV_core,parameter,ideoCyto)
stopCluster(cluster)

output_QC(info)
if(!parameter$IT_version){
  output_copy(alldata)
  output_distance(alldata)
}
