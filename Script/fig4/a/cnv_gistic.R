...
show_col(c("#F7B671","#EE7E32","#EF8E8F","#D8322F","#A1CF89","#38A245","#9BC6D4","#2B73A9","#B9A5C5","#5B448B","#848683"))
###set block color
mut<-c("High-level ampplification","Low-level amplification","Low-level deletion")
col<-c("#D8322F","#FFCCCC","#2B73A9")
coll <- col
...
Subdata<-function(df=df,gene=gene){
  ...
  df$class[df$type=="High-level ampplification"] <- 1
  df$class[df$type=="Low-level amplification"]<-2
  df$class[df$type=="Low-level deletion"]<-3
...
##############
cnv <- read.table("all_lesions.conf_95.txt",header=T,stringsAsFactors = F,sep = "\t")
cnv <- cnv[-grep("CN",cnv$Unique.Name),]
cnv <- cnv[which(cnv$q.values<0.05),]
amp <- cnv[grep("Amplification",cnv$Unique.Name),]
amp <- amp[,c(2,10:ncol(amp))]
amp <- melt(amp)
amp <- amp[,c(2,1,3)]
names(amp) <- c("sample","gene","mutation")
amp <- amp[-which(amp$mutation==0),]
amp$mutation[amp$mutation==1] <- 'Low-level amplification'
amp$mutation[amp$mutation==2] <- "High-level ampplification"
#############################################################################
del <- cnv[grep("Deletion",cnv$Unique.Name),]
del <- del[,c(2,10:ncol(del))]

del <- melt(del)
del <- del[,c(2,1,3)]
names(del) <- c("sample","gene","mutation")
del <- unique(del[-which(del$mutation==0),])
del$mutation[del$mutation==1] <- 'Low-level deletion'
###############################################################

F1 <- amp
F1$TYPE <- "Amplification"
names(F1) <- c("sample","gene","type","TYPE")

gene_freq_high <- Pick_topgene(df=F1[which(F1$type=="High-level ampplification"),],topgene = length(unique(F1$gene)))
gene_freq <- gene_freq_high
gene_freq_high$type <- "High-level ampplification"
gene_freq_low <- Pick_topgene(df=F1[which(F1$type=="Low-level amplification"),],topgene = length(unique(F1$gene)))
gene_freq_low$type <- "Low-level amplification"


F1 <- merge(F1,gene_freq,by="gene")
gene_freq$gene <- as.character(gene_freq$gene)
F1 <- Subdata(F1,gene_freq)
# gene_bar <- merge(gene_freq_high,gene_freq_low)
gene_freq_low <- gene_freq_low[match(gene_freq$gene,gene_freq_low$gene),]
#
F2 <- del
F2$TYPE <- "Deletion"
...
############RScluster end
pdf("AM_china_sv_complex_clin.pdf",width=12, height=10)

layout(matrix(c(1,2,3,4,5,5,6,7,7,8,9,9), 4, 3, byrow = TRUE),  heights=c(1,2.1,0.95,0.9),widths = c(7,1,1))

###amp
par(mar=c(0, 6.5, 1.8, 0), xpd=TRUE)
...
mtext(side = 2, text = "Amplification", line = 5, cex=1.2,font = 1)
# axis(4, at=1:nrow(gene_freq), labels=paste(rev(round(gene_freq$Freq/nrow(sample_order)*100,2)),"%",sep=""), line=0, tick=FALSE, las=1, cex.axis=1,font=1,mgp=c(3, 0, 0))
for(i in 0:nrow(sample_order)){
  lines(x=c(i-0.75,i-0.75),y=c(0.8,nrow(gene_freq)+0.8),col="black",lwd=0.5)
}
for(i in 0:nrow(gene_freq)){
  lines(x=c(-0.75,nrow(sample_order)-0.75),y=c(i+0.8,i+0.8),col="black",lwd=0.5)
}

#
par(mar=c(0.5, 0.6, 1, 0), xpd=TRUE)
barplot(-rev(gene_freq_high$Freq/55),beside=F,space=0.15,border = "NA",horiz = T,col=c("#D8322F"),las=1,yaxt='n',
        xlim=c(-0.6,0),xaxt='n')
axis(1, at=c(-0.6, -0.4, -0.2, 0), labels=c("0.6", "0.4", "0.2", "0"), las=1, cex.axis=1,font = 1);
#axis(4, at=loc2, labels=rev(row.names(F2)), line=FALSE, tick=FALSE, las=1, cex.axis=1.4,font=2)
mtext(side = 1, text = "High-level ampplification", line = 2, cex=0.7,font = 1)
text(x=-0.5,y=0.6+c(0:(nrow(gene_freq_high)-1))*1.15,
     labels =rev(paste0(sprintf("%0.2f",(gene_freq_high$Freq/55)*100),"%")),cex=0.8)
################################################amp
par(mar=c(0.5, 0, 1, 0.6), xpd=TRUE)
barplot(rev(gene_freq_low$Freq/55),beside=F,space=0.15,border = "NA",horiz = T,col=c("#FFCCCC"),las=1,yaxt='n',
        xlim=c(0,0.6),xaxt='n')
axis(1, at=c(0,0.2,0.4,0.6), labels=c("","0.2","0.4","0.6"), las=1, cex.axis=1,font = 1);
#axis(4, at=loc2, labels=rev(row.names(F2)), line=FALSE, tick=FALSE, las=1, cex.axis=1.4,font=2)
mtext(side = 1, text = "Low-level ampplification", line = 2, cex=0.7,font = 1)
text(x=0.5,y=0.6+c(0:(nrow(gene_freq_low)-1))*1.15,
     labels =rev(paste0(sprintf("%0.2f",(gene_freq_low$Freq/55)*100),"%")),cex=0.8)
##############delition
par(mar=c(0.1, 6.6, 3, 0), xpd=TRUE)
...
axis(2, at=(1:nrow(gene_freq2))+0.3, labels=rev(gene_freq2$gene), line=-1, tick=FALSE, las=1, cex.axis=1,font=1)
mtext(side = 2, text = "Delition", line = 5, cex=1.2,font = 1)
axis(4, at=(1:nrow(gene_freq2))+0.3, labels=paste(rev(round(gene_freq2$Freq/nrow(sample_order)*100,2)),"%",sep=""), line=0, tick=FALSE, las=1, cex.axis=1,font=1,mgp=c(3, 0, 0))
for(i in 0:nrow(sample_order)){
  lines(x=c(i-0.75,i-0.75),y=c(0.8,nrow(gene_freq2)+0.8),col="black",lwd=0.5)
}
for(i in 0:nrow(gene_freq2)){
  lines(x=c(-0.75,nrow(sample_order)-0.75),y=c(i+0.8,i+0.8),col="black",lwd=0.5)
}
#leng
par(mar=c(0.1, 0, 0,1), xpd=TRUE)
plot(x=5,y=8,cex=0,xlab = "",ylab = "",xaxt="n",yaxt="n",frame.plot=FALSE)
legend(4,8, pch=15,  col=mut_col$col[1:3],legend=mut_col$mut[1:3], title="", title.adj = 0, border=FALSE, cex=1.2,horiz=F,bty="n")

#clin
par(mar=c(0.1,8.3, 0, 1.5), xpd=TRUE)
...
legend(3,10, pch=15, col=c("#555555","#99CC99"),legend=c("non-WGD","WGD"), title="",title.adj = 0, border=FALSE, cex=1,horiz=F,ncol = 2,bty="n")
legend(3,9, pch=15, col=c("#ff8fa3","#c9184a","#590d22"),legend=c("<10","10-20",">20"), title="kataegeis",title.adj = 0, border=FALSE, cex=1,horiz=F,ncol = 3,bty="n")
legend(3,8, pch=15, col=c("#ff8fa3","#c9184a","#d3d3d3"),legend=c("low","high","Non"), title="chromothripsis",title.adj = 0, border=FALSE, cex=1,horiz=T,bty="n")
legend(3,7.5, pch=15, col=c("#CC9933","#D28EFF","#00A087FF","#6633FF"),legend=c("hand","heel","sole","other(foot)"), title="",title.adj = 0, border=FALSE, cex=1,horiz=F,ncol = 2,bty="n")
legend(3,6.5, pch=15, col=c("#FF33FF","#990066","#99FF99","#CC9999"),legend=c("BRAF","NRAS","NF1","Triple WT"), title="",title.adj = 0,text.width = 0.5, border=FALSE, cex=1,horiz=T,bty="n")
legend(3,5.8, pch=15,  col=c("#E64B35FF","#F39B7FFF","#4DBBD5FF","#00A087FF","#03827f","#f94144"),
       legend=c("A","B","C","D","E","F"),title="", title.adj = 0, border=FALSE,  cex=1,horiz = T,bty="n")
