...
signatures<-read.table("RS_cluster_vs_signature.txt",header = T,quote = "",sep = "\t")
...
variables <- c("cluster_A_B")
burden <- c("APOBEC","Unknown")
...
for(i in burden){
  p_list <- lapply(1:length(variables),function(x){
    ...
    if ( i == "Unknown" ){
    p <- p+labs(x="",y=paste0("Proportion of unknown signatures"),color="")+
    scale_color_manual(values=c("#8DAFDC","#506D9C"))
    }else if ( i == "APOBEC" ){
    p <- p+labs(x="",y=paste0("Proportion of APOBEC signatures"),color="")+
    scale_color_manual(values=c("#8DAFDC","#506D9C"))
    }
