...
signatures<-read.table("NRAS_BRAF_vs_SV.txt",header = T,quote = "",sep = "\t")
signatures$RS1_3_4<-rowSums(signatures[,c("RS1","RS3","RS4")])
signatures$RS2_5<-rowSums(signatures[,c("RS2","RS5")])


#signatures$SV_S4_flag <- ifelse(signatures$BRAFNRAS == 1,"BRAF/NRAS-driven tuomr","Non BRAF/NRAS-driven tuomr")

signatures=signatures[signatures$BRAFNRAS == 1,]
signatures$BRAFNRAS[signatures$BRAF_driven == 1] = "BRAF-driven tumor" 
signatures$BRAFNRAS[signatures$NRAS_driven == 1] = "NRAS-driven tumor" 

signatures$BRAFNRAS <-factor(signatures$BRAFNRAS,levels=c("BRAF-driven tumor","NRAS-driven tumor"))

table(signatures$BRAFNRAS)#Type	hypertension	anamnesis	MutNum

variables <- c("BRAFNRAS")
burden <- c("RS1_3_4","RS2_5")
...
for(i in burden){
  p_list <- lapply(1:length(variables),function(x){
    ...
    if ( i == "RS1_3_4"){
    p <- p+labs(x="",y=paste0("Proportion of RS1/3/4"),color="")+
    scale_color_manual(values=c("#8DAFDC","#506D9C"))
    }else if ( i == "RS2_5"){
    p <- p+labs(x="",y=paste0("Proportion of RS2/5"),color="")+
    scale_color_manual(values=c("#8DAFDC","#506D9C"))
    }
   ...
   ggsave(paste0("boxplot_",i,"_gene.pdf"), width = length(variables)*2.2, height = 3)# dev.off()
}
