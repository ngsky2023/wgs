...
signatures$BRAFNRAS <- ifelse(signatures$BRAFNRAS == 1,"BRAF/NRAS-driven tumor","Non BRAF/NRAS-driven tumor")

signatures$BRAFNRAS <-factor(signatures$BRAFNRAS,levels=c("Non BRAF/NRAS-driven tumor","BRAF/NRAS-driven tumor"))
  ...
  p_list <- lapply(1:length(variables),function(x){
    ...
    p <- ggplot()+
      ...
      theme(panel.border=element_blank(),
            ...
            axis.text.x=element_text(size=7,color='black',angle=-10,vjust = 0.5,hjust = 0.5),
            #axis.text.x=element_text(size=8,color='black'),
            axis.text.y=element_text(size=7,color='black'),
            axis.title=element_text(size=8,color='black'),
            plot.margin=margin(5.5,0,5.5,0,unit="pt"))
    if ( i == "RS4"){
    p <- p+labs(x="",y=paste0("Proportion of RS4"),color="")+
   ...
   ggsave(paste0("boxplot_",i,"_BRAFNRAS.pdf"), width = length(variables)*2.5, height = 3)# dev.off()
}
