...
table(tmb$Type)#Type	hypertension	anamnesis	MutNum
...
for(i in burden){
  ...
  ggsave(paste0("AM_boxplot_",i,"_clinical.pdf"), width = floor(length(variables)/3)*5, height = ceiling(length(variables)/9)*5)
  # dev.off()
...
##################################多组
library(reshape2)
data2 <- tmb[,c('sample','Primary.Site','perMb')]
data3 <- melt(data2,id.vars = c("sample","Primary.Site"),variable.name = "perMb",value.name = "number")
...
for(n in 1:length(burden)){
  ...
  my_comparisons=list(c("hand","heel"),c("hand","other(foot)"),c("hand","sole"),c("sole","heel"),c("sole","other(foot)"),c("heel","other(foot)")) 
  symnum.args <- list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1),
                      symbols = c("****", "***", "**", "*", "ns"))
  
  g <- ggboxplot(data,"Primary.Site","number",palette = 
                   c("#CC9933","#D28EFF","#00A087FF","#6633FF"),
                 fill = "Primary.Site" ,facet.by = "perMb") +
    # geom_jitter(data=data,aes(x=as.character(data[,clin]),y=data[,i],color=data[,clin]),width=0.15,size=0.6)+
    stat_compare_means(comparisons = my_comparisons,method="wilcox.test",)+
    stat_compare_means(label.y=ymax+10)+
    # stat_compare_means(comparisons=my_comparisons,method="wilcox.test")+ # #,aes(label=..p.signif..)
    labs(x=i,y="Mutation per megabase",color="")+
    # scale_fill_lancet()+
  ...
  ggsave(paste0("AM_boxplot_Site_",i,"_clinical.pdf"),width = 6,height = 6)
}
