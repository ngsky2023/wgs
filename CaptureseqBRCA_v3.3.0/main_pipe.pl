...
if (!$que){
	...
	foreach my $sample(@samples){
		...
		system("nohup perl $Bin/qqsub.pl -que $que -inputdir $outdir -sample $sample -step $step -type $Interpretation -cut 2 1>$sample.sh.o 2>$sample.sh.e &");
	}
}
my @recall_mark;
my @sample_effec;
...
foreach my $sample(@samples){
        if (-f "$outdir/$sample/2.Realign/recall_$sample.mark"){
                my $fmark = "$outdir/$sample/2.Realign/recall_$sample.mark";
                unless (grep /$fmark/,@recall_mark){
                        push (@recall_mark,$fmark);
                }
        }
}
if ( $#recall_mark ==  $#samples){
        print LOG "@samples\n@recall_mark\n";
        foreach my $sample(@samples){
        if (-f "$outdir/$sample/2.Realign/recall_$sample.mark"){
                my $sample_dir = "$outdir/$sample/6.QC/QC";
                my ($Average_depth,$Coverage_1X,$Coverage_10X,$Coverage_20X)= map {0} 1..4;
                opendir (DIN,$sample_dir)  or die "Couldn't open $sample_dir, $!";
                my $QC_file = "$sample\_QC.xls";
                if (!-f "$sample_dir/$QC_file" ) {
                        print  LOG "ERROR:Do not creat $sample_dir/$QC_file!\n";
                }else{
                        open QCF,"<$sample_dir/$QC_file" or die "Couldn't open $QC_file,$!";
                        while(<QCF>){
                                chomp;
                                next if ($_ =~ /^\s*$/);
                                my @dat = split /\t/,$_;
                                if ($dat[0] eq "Average sequencing depth on target"){
                                        $Average_depth = $dat[1];
                                }
                                if ($dat[0] eq "Coverage of target region"){
                                        $Coverage_1X = $dat[1];
                                }
                                if ($dat[0] eq "Fraction of target covered at least 10x"){
                                        $Coverage_10X = $dat[1];
                                }
                                if ($dat[0] eq "Fraction of target covered at least 20x"){
                                        $Coverage_20X = $dat[1];
                                }
                        }
                        close QCF;
                        ...
                        $Coverage_10X =~ s/\%//;
                        $Coverage_20X =~ s/\%//;
                        if ($Average_depth > 200 && $Coverage_1X > 98 && $Coverage_10X > 95 && $Coverage_20X > 90){
                                ...
                                unless (grep /<$sample>/,@sample_effec){
                                        push (@sample_effec,$sample);
                                }
                        }
                }
                closedir DIN;
        ...
        print LOG "@sample_effec\n";
        foreach my $sample(@sample_effec){
                ###  qsub shell
                if (!$que){
                	#foreach my $sample(@samples){
                		system("nohup perl $Bin/qsub.pl -inputdir $outdir -sample $sample -step $step -type $Interpretation -cut $cut 1>>$sample.sh.o 2>>$sample.sh.e &");
                	...
                	#foreach my $sample(@samples){
                                ...
                                system($pskill);
                                #`ps -ef |grep "$outdir" |grep "$sample" |cut -c 10-15|xargs -L 1 kill`;
                		system("nohup perl $Bin/qqsub.pl -que $que -inputdir $outdir -sample $sample -step 3 -type $Interpretation -cut $cut 1>>$sample.sh.o 2>>$sample.sh.e &");
                	#}
                }
        }
