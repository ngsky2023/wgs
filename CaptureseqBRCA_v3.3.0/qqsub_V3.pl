...
if ($step <=0 && $cut>=0){
        ...
        print "$cmd\n";
	system("cd $shell_dir/Input_fq/1.Trim/ && $qsub -l p=1 -l vf=4g Preprocess_$sample.sh");
	$finish = 1;	
	while($finish){
		sleep 60;
...
if ($step <=1 && $cut>=1){
	foreach my $rmdup(glob "$shell_dir/$sample/1.Map/bam_*.sh"){
	...
	while($finish){
		...
		if (-e "$shell_dir/$sample/1.Map/rmdup_$sample.mark"){
			die "$sample.sort.Header.bam not exists!\n" unless (-e "$shell_dir/$sample/1.Map/$sample.sort.Header.bam");	
			$finish = 0;
		}
	}
...
if ($step <=2 && $cut>=2){
	if (-e "$shell_dir/$sample/2.Realign"){
                 ...
                 print "$cmd\n";
		system("cd $shell_dir/$sample/2.Realign && $qsub -l p=1 -l vf=13g  GATK_recall_$sample.sh");
		$finish = 1;
		while($finish){
			sleep 60;
...
if ($step<=3 && $cut>=3){
        $cmd="cd $shell_dir/$sample/3.Variants/snp && $qsub -l p=1 -l vf=12g   verscan_$sample.sh";
        print "$cmd\n";
        $cmd="cd $shell_dir/$sample/3.Variants/indel && $qsub -l p=1 -l vf=12g verscan_$sample.sh";
        print "$cmd\n";
        system("cd $shell_dir/$sample/3.Variants/snp && $qsub -l p=1 -l vf=12g   verscan_$sample.sh");
        system("cd $shell_dir/$sample/3.Variants/indel && $qsub -l p=1 -l vf=12g  verscan_$sample.sh");
        $finish = 1;
        while($finish){
                sleep 60;
                if (-e "$shell_dir/$sample/3.Variants/snp/verscan_$sample.mark" && -e "$shell_dir/$sample/3.Variants/indel/versacn_$sample.mark"){
                        $finish = 0;
                }else{
                        $finish = 1;
                }
