...
";
my ($shell_dir,$sample,$step,$cut,$help,$node,$type,$finish);
GetOptions(
...
my $cmd="";
my $qsub="qsub -cwd ";
$qsub = "$qsub -l h=$node " if ($node);
if (!$shell_dir || !$sample || $help){
	die "$usage\n";
}
die "start-step must bigger than the end-step\n" if ($step > $cut);
if ($step <=0 && $cut>=0){
        $cmd="cd $shell_dir/Input_fq/1.Trim/ && $qsub -l p=1 -l vf=8g Preprocess_$sample.sh";
        print "$cmd\n";
	system("cd $shell_dir/Input_fq/1.Trim/ && $qsub -l p=1 -l vf=8g Preprocess_$sample.sh");
	$finish = 1;	
	while($finish){
		sleep 60;
...
if ($step <=1 && $cut>=1){
	foreach my $rmdup(glob "$shell_dir/$sample/1.Map/bam_rmdup_*.sh"){
		my $dirname = dirname $rmdup;
                ...
                print "$cmd\n";
		system("cd $dirname && $qsub -l p=1 -l vf=8g $rmdup");
	}
	$finish = 1;
	while($finish){
		sleep 60;
...
if ($step <=2 && $cut>=2){
	if (-e "$shell_dir/$sample/2.Realign"){
                 $cmd="cd $shell_dir/$sample/2.Realign && $qsub -l p=1 -l vf=8g  GATK_recall_$sample.sh";
                 print "$cmd\n";
		system("cd $shell_dir/$sample/2.Realign && $qsub -l p=1 -l vf=8g  GATK_recall_$sample.sh");
		$finish = 1;
		while($finish){
			sleep 60;
...
if ($step<=3 && $cut>=3){
        ...
        print "$cmd\n";
        $cmd="cd $shell_dir/$sample/3.Variants/indel && $qsub -l p=1 -l vf=10g  indel_platypus_$sample.sh";
        print "$cmd\n";
        ...
        system("cd $shell_dir/$sample/3.Variants/indel && $qsub -l p=1 -l vf=10g  indel_platypus_$sample.sh");
        $finish = 1;
        while($finish){
                sleep 60;
...
if($type !~ /Gynecological/){
	if ($step <=4 && $cut>=4){
		$cmd="cd $shell_dir/$sample/4.Annot && $qsub -l p=1 -l vf=3g annotate.indel.$sample.sh";
                print "$cmd\n";
		$cmd="cd $shell_dir/$sample/4.Annot && $qsub -l p=1 -l vf=3g annotate.snp.$sample.sh";
                print "$cmd\n";
		system("cd $shell_dir/$sample/4.Annot && $qsub -l p=1 -l vf=3g annotate.indel.$sample.sh");
		system("cd $shell_dir/$sample/4.Annot && $qsub -l p=1 -l vf=3g annotate.snp.$sample.sh");
		$finish = 1;
		while($finish){
			sleep 60;
