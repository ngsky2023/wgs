...
foreach my $sample(@samples){
	...
	system("perl $Bin/Map_Realign.fast.pl -sample $sample  -read1  $Info{$sample}[1] -read2  $Info{$sample}[2] -outdir $outdir -ref $ref -bwa $bwa -samtools $samtools -interpretation $Interpretation  -picard $picard -bedtools $bedtools -bed $bed -BwaParam '$bwa_param_mem' -gatk $gatk  -dbsnp $dbsnp");
