...
while(my $line = <CONFIG>){
	...
	if ($line =~ /verscan\s*=\s*(.*)$/i){$verscan = $1;next;}
...
foreach my $sample(@samples){
	...
	if($Interpretation =~ /fast/){
		...
		system("perl $Bin/Variants_calling_verscan.pl -infile  $outdir/$sample/2.Realign/$sample.bam  -outdir $outdir/$sample  $call_variant -ref $ref2 -bed $bed -gatk $gatk -samtools $samtools -bcftools $bcftools -MustGiven $MustGiven -ruby $ruby -ASNP $ASNP -AINDEL $AINDEL -Platypus  $Platypus");
	}else{
		system("perl $Bin/Variants_calling_verscan.pl -infile  $outdir/$sample/2.Realign/$sample.bam  -outdir $outdir/$sample  $call_variant -ref $ref -bed $bed -gatk $gatk -samtools $samtools -bcftools $bcftools -MustGiven $MustGiven  -ruby $ruby -ASNP	$ASNP -AINDEL $AINDEL -Platypus  $Platypus");
	}
	### step IV annotation ###
