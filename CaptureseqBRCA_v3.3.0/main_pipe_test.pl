...
###read config#######
my ($ref,$bwa,$samtools,$picard,$bwa_param_mem,$gatk,$annovar,$dbsnp,$bed,$bed2,$bcftools,$bedtools,$call_variant,$software,$gene_info,$ruby,$ASNP,$AINDEL,$MustGiven,$Xindel,$hgvs,$Platypus,$flexbar,$rd,$adapter,$Interpretation,$random);
...
foreach my $sample(@samples){
	...
	system("perl $Bin/Map_Realign_test.pl -sample $sample  -read1  $Info{$sample}[1] -read2  $Info{$sample}[2] -outdir $outdir -ref $ref -bwa $bwa -samtools $samtools -interpretation $Interpretation  -picard $picard -bedtools $bedtools -bed $bed -BwaParam '$bwa_param_mem' -gatk $gatk  -dbsnp $dbsnp");
	### step III variants calling 
	if($Interpretation =~ /fast/){
		my $ref2="$ref.genome.fa";
		system("perl $Bin/Variants_calling.pl -infile  $outdir/$sample/1.Map/$sample.sort.Header.bam  -outdir $outdir/$sample  $call_variant -ref $ref2 -bed $bed -gatk $gatk -samtools $samtools -bcftools $bcftools -MustGiven $MustGiven -ruby $ruby -ASNP $ASNP -AINDEL $AINDEL -Platypus  $Platypus");
	}else{
		system("perl $Bin/Variants_calling.pl -infile  $outdir/$sample/1.Map/$sample.sort.Header.bam  -outdir $outdir/$sample  $call_variant -ref $ref -bed $bed -gatk $gatk -samtools $samtools -bcftools $bcftools -MustGiven $MustGiven  -ruby $ruby -ASNP	$ASNP -AINDEL $AINDEL -Platypus  $Platypus");
	}
	### step IV annotation ###
