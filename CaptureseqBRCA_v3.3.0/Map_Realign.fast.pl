...
sub mapping{
	...
	print DUP "echo \"$sample rmdup finish\" >rmdup_$sample.mark\n";
...
sub realign{
	...
	print RECALL "echo ========== GATK Realigner and BaseRecall start at : \`date\`==========\n";
        my $realn_region_opt = " -L $bed --interval_padding 100 ";
	if($interpretation=~/fast/){
		print RECALL "java -Xmx5G -jar $GATK -T RealignerTargetCreator $realn_region_opt -nt 2 -R $ref2 -I $info -o $outdir/$sample/2.Realign/$sample.intervals -known $snp_vcf\n";
		print RECALL "java -jar $GATK -T IndelRealigner  $realn_region_opt -R $ref2 -targetIntervals $outdir/$sample/2.Realign/$sample.intervals -I $info -o $outdir/$sample/2.Realign/$sample.realn.bam -known $snp_vcf\n";
		print RECALL "java -jar $GATK -T BaseRecalibrator $realn_region_opt -R $ref2 -I $outdir/$sample/2.Realign/$sample.realn.bam -knownSites $snp_vcf -o $outdir/$sample/2.Realign/$sample.grp\n";
		...
		print RECALL "java -Xmx5G -jar $GATK -T RealignerTargetCreator $realn_region_opt -nt 2 -R $ref -I $info -o $outdir/$sample/2.Realign/$sample.intervals -known $snp_vcf\n";
                print RECALL "java -jar $GATK -T IndelRealigner $realn_region_opt -R $ref -targetIntervals $outdir/$sample/2.Realign/$sample.intervals -I $info -o $outdir/$sample/2.Realign/$sample.realn.bam -known $snp_vcf\n";
                print RECALL "java -jar $GATK -T BaseRecalibrator $realn_region_opt  -R $ref -I $outdir/$sample/2.Realign/$sample.realn.bam -knownSites $snp_vcf -o $outdir/$sample/2.Realign/$sample.grp\n";
