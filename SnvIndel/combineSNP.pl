...
my ($bloc , $bend , $baltDep , $brate) = (0) x 4;
my $id = '';
my $n = 1;
my $sample;
open IN , "$input";
...
while (<IN>){
	...
	chomp;
	my @F = split /\t/ , $_;
	my ($chr , $loc , $end , $ref , $alt) = @F[1,2,3,4,5];
	$sample = $F[0];
	my $type;
	if (length($ref) == 1 and length($alt) == 1 and $ref ne "-" and $alt ne "-"){
		$type = 'SNP';
	}else{
		$type = 'INDEL';
	}
	...
	if ($bchr ne $chr
		...
		if ($bchr){
			print "$sample\t$bchr\t$bloc\t$bend\t$bref\t$balt\t$binfo\n";
		}
		($bchr , $bloc , $bend , $bref , $balt , $baltDep , $brate , $btype , $binfo) = ($chr , $loc , $end , $ref , $alt , $dalt , $rate , $type , join("\t" , @F[6..$#F]));
		if ($id eq '.'){
			$id = "Mut$n";
			$n++;
		}
